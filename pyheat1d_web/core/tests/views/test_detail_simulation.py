...
def test_table_value(client, simulation):
    ...
    assertContains(resp, f"{simulation.length}".replace(".", ","))
    assertContains(resp, simulation.ndiv)
    assertContains(resp, simulation.nstep)
    assertContains(resp, f"{simulation.dt}".replace(".", ","))
    assertContains(resp, f"{simulation.initialt}".replace(".", ","))
    assertContains(resp, f"{simulation.lbc_value}".replace(".", ","))
    assertContains(resp, f"{simulation.rbc_value}".replace(".", ","))
